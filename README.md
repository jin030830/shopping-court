...
이 프로젝트는 SPA(Single Page Application)이므로 정적 호스팅 시 라우팅 설정이 필요합니다:
- **Vercel**: `vercel.json` 파일이 포함되어 있어 자동으로 설정됩니다.
- **Netlify**: `_redirects` 파일을 사용할 수 있습니다.
- **기타**: 모든 경로를 `index.html`로 리다이렉트하는 설정이 필요합니다.