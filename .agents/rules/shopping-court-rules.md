...
* **주요 흐름**: 닉네임 생성 → 게시글 탐색 및 상세 진입 → 투표 참여 → 댓글 작성 및 토론 → 120시간 후 판결 확정 및 결과 열람 → **판사봉 획득 및 포인트 교환**.
...
* **판결 확정 시스템**:
    * 게시글은 작성 시점부터 120시간 동안만 활성 상태를 유지합니다.
    * 120시간 경과 시 자동으로 '판결 완료' 상태로 전환되며, 투표와 댓글 작성이 비활성화됩니다.
    * 판결 결과는 다수결에 따라 유죄/무죄/판결 보류(동률 시)로 결정되며 도장 형태로 표시됩니다.
* **인기 게시글(실시간 인기 투표)**:
    * **산정 공식**: 조회수.
    * 지표가 높은 글 top3은 홈 화면 상단 '실시간 인기 투표(화제의 재판)' 영역에 노출됩니다.
