...
export const tossLogout = functions
  ...
  .https.onCall(async (data: { userKey?: string }) => {
    try {
      ...
      // 개발자 모드 사용자는 로그아웃 API 호출 스킵
      if (data.userKey.startsWith('dev-user-') || data.userKey.startsWith('test-user-')) {
        console.log(`👨‍💻 개발자/테스트 모드 사용자 로그아웃: ${data.userKey}`);
        return { success: true, message: '개발자 모드 로그아웃 완료' };
      }

      const config = getTossApiConfig();
      const httpsAgent = createMtlsAgent();

      // 토스 API의 "로그인 연결 끊기" 호출 (userKey 사용)
      ...
      console.log('✅ 토스 로그인 연결 끊기 성공:', response.data);
      return { success: true, data: response.data };
    } catch (error: any) {
      console.error("토스 로그아웃 처리 오류:", error);
      if (error.response) {
        console.error("응답 데이터:", error.response.data);
        console.error("응답 상태:", error.response.status);
      }
      // 로그아웃 실패해도 Firebase 로그아웃은 진행되도록 에러를 던지지 않음
      console.warn("⚠️ 토스 로그아웃 실패했지만 계속 진행합니다.");
      return { success: false, message: "토스 로그아웃 실패" };
    }
  });
